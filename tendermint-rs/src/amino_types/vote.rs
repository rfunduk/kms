...
impl SignableMsg for SignVoteRequest {
        ...
        B: BufMut,
    {
        ...
        println!("STATSD///tmkms.{:?}.height:{:?}|g", chain_id.to_string(), cv.height);
        let hwm_filename = format!("{}/.tmkms/hwm-vote-{}", env::var("HOME").unwrap(), chain_id.to_string());
        if Path::new(&hwm_filename.clone()).exists() {
          ...
          if !ok_to_sign {
            ...
            println!("STATSD///tmkms.{:?}.refused-vote:1|c", chain_id.to_string());
            return Ok(false);
          }
        }
        println!("Signing vote at {:?} round {:?} (type: {:?})", cv.height, cv.round, cv.vote_type);
        ...
        println!("STATSD///tmkms.{:?}.signed-vote:1|c", chain_id.to_string());
