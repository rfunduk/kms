...
impl SignableMsg for SignProposalRequest {
        ...
        B: BufMut,
    {
        ...
        println!("STATSD///tmkms.{:?}.height:{:?}|g", chain_id, cp.height);
        ...
        if Path::new(&hwm_filename.clone()).exists() {
          ...
          if !ok_to_sign {
            ...
            println!("STATSD///tmkms.{:?}.refused-proposal:1|c", chain_id);
            return Ok(false);
          }
        }
        println!("Signing proposal at {:?} round {:?} (pol round: {:?}) (type: {:?})", cp.height, cp.round, cp.pol_round, cp.msg_type);
        ...
        println!("STATSD///tmkms.{:?}.signed-proposal:1|c", chain_id);
